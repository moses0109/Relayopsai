...
const MedSpaIncomingCall: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Show first call after 6 seconds
    const showTimer = setTimeout(() => setShowCall(true), 6000);

    // Hide after 5 seconds of showing
    const hideTimer = setTimeout(() => setShowCall(false), 11000);

    // Cycle only 2 more times, every 45 seconds
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 sm:bottom-8 sm:right-8 z-50 transition-all duration-500 ease-out max-w-[calc(100vw-2rem)] ${
        showCall
    ...
    >
      ...
      <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-2xl shadow-slate-900/50 border border-rose-500/40 w-64 sm:w-80 backdrop-blur-xl transition-all duration-300">
        ...
        <div className="mb-4">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 sm:w-12 sm:h-12 bg-gradient-to-br from-rose-500 to-pink-600 rounded-full flex items-center justify-center flex-shrink-0">
          ...
          <div className="bg-rose-500/20 border border-rose-500/40 rounded-xl px-4 py-3 flex items-center justify-between">
            <div>
              ...
              <p className="text-white text-base sm:text-xl font-black">{call.value}</p>
            </div>
